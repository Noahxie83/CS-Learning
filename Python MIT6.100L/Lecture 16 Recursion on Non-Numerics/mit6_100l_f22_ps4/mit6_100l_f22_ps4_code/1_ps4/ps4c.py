...
def decrypt_message_try_pads(ciphertext,pads):
    ...
    most_valid_words=-1
    for pad in pads:
        plaintext_message=ciphertext.decrypt_message(pad)
        valid_word_count=0
        for word in plaintext_message.get_text().split():
            if is_word(wordlist,word):
                valid_word_count+=1
        if valid_word_count>=most_valid_words:
            most_valid_words=valid_word_count
            best_message=plaintext_message
...
def decode_story():
    ...
    pads=get_story_pads()
    plaintext_message=decrypt_message_try_pads(ciphertext,pads)
    return plaintext_message.get_text()  # delete this line and replace with your code here
