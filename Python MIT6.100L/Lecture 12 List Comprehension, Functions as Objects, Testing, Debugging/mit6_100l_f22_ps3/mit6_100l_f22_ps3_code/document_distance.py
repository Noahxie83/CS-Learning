...
def get_most_frequent_words(freq_dict1, freq_dict2):
    ...
    for k,v in freq_dict2.items():
        if k not in freq_all:
            ...
            freq_all[k]+=v
    max_feq=max(freq_all.values())
    ret=[]
    for k,v in freq_all.items():
        if v==max_feq:
            ret.append(k)
    return sorted(ret)
    ...
    for file_path in file_paths:
        ...
        for word in frequencies:
            if word not in document_counts:
                ...
                document_counts[word] += 1
    idf_dict = {}
    for word, document_count in document_counts.items():
        idf_dict[word] = math.log10(
...
def get_tfidf(tf_file_path, idf_file_paths):
    ...
    tfidf_list.sort(key=lambda item: (item[1], item[0]))
    return tfidf_list
