...
def filter(pixels_list, color):
    ...
    for pixel in pixels_list:
        values = matrix_multiply(transformation, pixel)
        values = tuple(max(0, min(255, int(value))) for value in values)
        transformed_pixels.append(values)
...
def extract_end_bits(num_end_bits, pixel):
    ...
    """
    modulus = 2 ** num_end_bits
    if isinstance(pixel, tuple):
        return tuple(channel % modulus for channel in pixel)
    return pixel % modulus
...
def reveal_bw_image(filename):
    ...
    with Image.open(filename) as source:
        ...
        size = image.size
    revealed = [255 * extract_end_bits(1, pixel) for pixel in pixels]
    return pix_to_img(revealed, size, "L")
...
def reveal_color_image(filename):
    ...
    for pixel in pixels:
        ...
        revealed.append(tuple(int(channel * 255 / 7) for channel in bits))
    return pix_to_img(revealed, size, "RGB")
...
def draw_kerb(filename, kerb):
    ...
    draw.text((0, 0), kerb, fill="white", font=font)

    path = Path(filename)
    output = path.with_name(path.stem + "_kerb" + path.suffix)
    image.save(output)



def main():
    original = Image.open("image_15.png")
    filtered_pixels = filter(img_to_pix("image_15.png"), "red")
    filtered = pix_to_img(filtered_pixels, original.size, "RGB")
    filtered.save("image_15_red_filtered.png")
    original.close()

    reveal_image("hidden1.bmp").save("hidden1_revealed.bmp")
    reveal_image("hidden2.bmp").save("hidden2_revealed.bmp")

    print("Saved image_15_red_filtered.png, hidden1_revealed.bmp, and hidden2_revealed.bmp")
    print("Call draw_kerb(...) manually when you are ready to create submission images.")
